...
import { ArrowLeft, ArrowRight, CalendarDays, CheckCircle2, Clock3, Mail, MapPin, Phone, Search, Upload } from 'lucide-react';
...
export function TrackComplaintPage() { const [params] = useSearchParams(); const [query, setQuery] = useState(params.get('code') || ''); const [result, setResult] = useState(null); const [searched, setSearched] = useState(false); const [loading, setLoading] = useState(false); const find = (event) => { event.preventDefault(); if (!query.trim()) return; setLoading(true); setSearched(false); window.setTimeout(() => { const value = query.trim().toLocaleLowerCase(); setResult(complaintEntries.find((item) => item.code.toLocaleLowerCase() === value || item.title.toLocaleLowerCase().includes(value)) || null); setLoading(false); setSearched(true); }, 500); }; return <>
...
function ComplaintResult({ result }) { return (
  <div className="tracking-result-v3">
    ...
    <Link className="citizen-button citizen-button-primary" to={`/cong-dong/tra-cuu/${result.code}`}>Xem chi tiết tiến độ <ArrowRight size={16} /></Link>
  </div>
); }
