...
export const createWorkScheduleSchema = yup.object().shape({
    ...
    batDau: yup
        ...
        .test("validTime", "Giờ bắt đầu không hợp lệ", (value) => {
            ...
            if (timeRegex24.test(value)) return true;
            const timeRegex12 = /^(\d{1,2}):(\d{2})\s*(SA|CH)$/i;
            if (timeRegex12.test(value)) {
                const converted = convertTo24Hour(value);
                return timeRegex24.test(converted);
            }
            return false;
        })
        .test(
        ...
        .test(
            ...
            function (value) {
                ...
                const startTime24h = (value.includes('CH') || value.includes('SA')) 
                    ? convertTo24Hour(value) 
                    : value;
                const endTime24h = (ketThuc.includes('CH') || ketThuc.includes('SA')) 
                    ? convertTo24Hour(ketThuc) 
                    : ketThuc;
    ...
    ketThuc: yup
        ...
        .test(
            ...
            function (value) {
                ...
                const startTime24h = (batDau.includes('CH') || batDau.includes('SA')) 
                    ? convertTo24Hour(batDau) 
                    : batDau;
                const endTime24h = (value.includes('CH') || value.includes('SA')) 
                    ? convertTo24Hour(value) 
                    : value;
