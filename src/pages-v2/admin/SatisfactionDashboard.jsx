...
export default function SatisfactionDashboard() {
  ...
  const filtered = ratings.filter((item) => {
    ...
    const dateOk = dateFilter === 'all' || (dateFilter === 'recent' ? item.ratedAt >= '2026-07-10' : item.ratedAt < '2026-07-10');
  ...
  return <main className="p-6 md:p-8 bg-gray-50 min-h-screen">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-5 flex flex-wrap gap-3"><select className="border border-gray-300 rounded-lg px-3 py-2 text-sm" value={scoreFilter} onChange={(event) => setScoreFilter(event.target.value)}><option value="all">Tất cả số sao</option>{stars.map((item) => <option key={item} value={item}>{item} sao</option>)}</select><select className="border border-gray-300 rounded-lg px-3 py-2 text-sm" value={dateFilter} onChange={(event) => setDateFilter(event.target.value)}><option value="all">Tất cả ngày đánh giá</option><option value="recent">Từ 10/07/2026</option><option value="older">Trước 10/07/2026</option></select><span className="text-sm text-gray-500 self-center">{filtered.length} kết quả</span></div>
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden"><div className="px-5 py-4 border-b border-gray-200"><h2 className="font-semibold text-gray-900">Danh sách đánh giá</h2></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50 text-gray-500"><tr><th className="text-left px-5 py-3">Mã phản ánh</th><th className="text-left px-5 py-3">Điểm</th><th className="text-left px-5 py-3">Mức đánh giá</th><th className="text-left px-5 py-3">Ngày đánh giá</th><th className="text-left px-5 py-3">Góp ý</th></tr></thead><tbody className="divide-y divide-gray-100">{filtered.map((item) => <tr key={item.id}><td className="px-5 py-4 font-medium text-blue-600">{item.code}<div className="text-xs text-gray-500 font-normal mt-1">{complaintStatuses[item.code]?.title || 'Phản ánh mẫu'}</div></td><td className={`px-5 py-4 font-semibold ${scoreColor(item.score)}`}>{'★'.repeat(item.score)} <span className="text-gray-500">({item.score}/5)</span></td><td className="px-5 py-4">{satisfactionLabel(item.score)}</td><td className="px-5 py-4 text-gray-600">{item.ratedAt}</td><td className="px-5 py-4 max-w-sm text-gray-600">{item.comment || 'Không có góp ý'}</td></tr>)}</tbody></table></div></div>
    </div>
  </main>;
}
