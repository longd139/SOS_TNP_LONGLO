...
import { 
  ...
  LockOutlined, CheckOutlined, CloseOutlined, CloseCircleOutlined, EllipsisOutlined, GlobalOutlined
...
const legalDocsMock = [
  ...
  },
  {
    ...
    title: 'Báo cáo tổng kết công tác cải cách hành chính Quý 2',
    docType: 'Báo cáo',
    issueDate: '2026-06-15',
    status: 'Chờ duyệt',
    securityLevel: 'Nội bộ',
    fileName: 'BaoCao_CCHC_Q2.pdf',
    fileSize: '1.8 MB',
    approver: null,
    aiLearned: false,
    isEditing: false
...
const LegalDocs = () => {
  ...
  const [data, setData] = useState(() => {
    const saved = localStorage.getItem('legalDocsData_v2');
    return saved ? JSON.parse(saved) : legalDocsMock;
  });
  ...
  React.useEffect(() => {
    localStorage.setItem('legalDocsData_v2', JSON.stringify(data));
  }, [data]);
  ...
  const [docTypes, setDocTypes] = useState(() => {
    const saved = localStorage.getItem('legalDocTypes_v2');
    return saved ? JSON.parse(saved) : [
      'Nghị quyết', 'Nghị định', 'Quyết định', 'Chỉ thị', 'Thông tư', 
      'Báo cáo', 'Kế hoạch', 'Quy trình nghiệp vụ', 'Hướng dẫn', 'Chỉ đạo điều hành'
    ];
  });
  const [newDocType, setNewDocType] = useState('');

  React.useEffect(() => {
    localStorage.setItem('legalDocTypes_v2', JSON.stringify(docTypes));
  }, [docTypes]);
  ...
  const addDocType = (e) => {
    ...
    if (newDocType && !docTypes.includes(newDocType)) {
      ...
      setNewDocType('');
      message.success(`Đã thêm loại văn bản mới: "${newDocType}"`);
    }
  };

  const deleteDocType = (typeToDelete) => {
    if (docTypes.length <= 1) {
      message.warning('Phải giữ lại ít nhất 1 loại văn bản!');
      return;
    }
    setDocTypes(prev => prev.filter(type => type !== typeToDelete));
    message.success(`Đã xóa loại văn bản: "${typeToDelete}"`);
  };
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return matchText && matchType && matchStatus && matchDate && matchAI && matchSecurity;
    });
  }, [data, searchText, filterDocType, filterStatus, filterDate, filterAI, filterSecurity]);
  ...
  const columns = [
    ...
    {
      title: 'Trích yếu nội dung (Tiêu đề)',
      dataIndex: 'title',
      key: 'title',
      width: '35%',
      render: (text, record) => (
    ...
    },
    {
      ...
      render: (type) => {
        let color = 'blue';
        if (type === 'Quyết định') color = 'magenta';
        if (type === 'Chỉ đạo điều hành') color = 'orange';
        if (type === 'Kế hoạch') color = 'purple';
        return (
    ...
    form.validateFields().then((values) => {
      ...
      if (isEdit) {
        ...
        if (!isUserLeader) {
          ...
          addNotification('Yêu cầu duyệt chỉnh sửa', `Cán bộ ${currentUser?.fullName || ''} đã chỉnh sửa văn bản Pháp luật "${values.title}" và đang chờ duyệt lại.`, 'LEADER');
        } else {
        ...
        message.success('Tải lên tài liệu quy phạm pháp luật thành công!');
        if (!isLeader) {
          addNotification('Yêu cầu duyệt tài liệu mới', `Cán bộ ${currentUser?.fullName || ''} đã tải lên văn bản Pháp luật mới "${values.title}" và đang chờ duyệt.`, 'LEADER');
        }
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div>
          <Title level={4} className="text-blue-900 font-bold m-0 flex items-center gap-2">
            ...
            <span>Văn bản Quy phạm Pháp luật & Chỉ đạo Điều hành</span>
          </Title>
          <p className="text-xs md:text-sm text-gray-500 mt-1 m-0">Quản lý các nghị định, quyết định, chỉ đạo điều hành và kế hoạch chính thức.</p>
        </div>
        <Button 
          type="primary" 
          icon={<PlusOutlined />} 
      ...
      <Card className="shadow-sm border border-gray-200 rounded-lg" styles={{ body: { padding: 16 } }}>
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex flex-wrap gap-2 items-center w-full md:w-auto justify-end">
            ...
            {(searchText || filterDocType || filterStatus || filterDate || filterAI || filterSecurity) && (
              <Button 
                type="text" 
                danger
                onClick={() => {
      ...
      <Modal
        title={
          <div className="flex items-center gap-2 text-blue-900 border-b pb-3 mb-2 font-bold text-base">
            ...
            <span>{form.getFieldValue('id') ? "Cập nhật thông tin văn bản" : "Tải lên văn bản Quy phạm Pháp luật"}</span>
          </div>
        }
        open={isModalVisible}
        onOk={handleOk}
                    ...
                    {isLeader && (
                      <>
                        ...
                        <Space style={{ padding: '0 8px 4px' }}>
                          <Input
                            placeholder="Thêm loại văn bản..."
                            value={newDocType}
                            onChange={(e) => setNewDocType(e.target.value)}
                            onKeyDown={(e) => e.stopPropagation()}
                            className="rounded h-8"
                          />
                          <Button type="text" icon={<PlusOutlined />} onClick={addDocType} className="text-blue-600 font-bold h-8">
                            Thêm
                          </Button>
                        </Space>
                      </>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 border-b pb-1 mb-0">
                Thông tin văn bản quy phạm
              </h4>
              
              <div className="grid grid-cols-3 gap-y-2.5 text-xs">
                <span className="text-gray-500">Số / Ký hiệu:</span>
                <span className="col-span-2 font-bold text-blue-600">{selectedDoc.docNumber || 'N/A'}</span>

                <span className="text-gray-500">Loại văn bản:</span>
                <span className="col-span-2 text-slate-700 font-semibold">{selectedDoc.docType || 'N/A'}</span>

                <span className="text-gray-500">Ngày ban hành:</span>
                <span className="col-span-2 text-slate-700 font-semibold">{selectedDoc.issueDate}</span>
