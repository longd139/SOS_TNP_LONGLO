...
export default function ReceptionKiosk() {
  ...
  const submitFeedback = () => {
    const payload = { id: `TD-${Date.now()}`, type: 'TIEP_DAN', feedbackType: activeSession?.feedbackType, receptionId: activeSession?.receptionId, ticketNo: activeSession?.ticketNo, receptionDate: activeSession?.date, weekday: activeSession?.weekday, receptionSlot: activeSession?.slot, topic: activeSession?.topic, description: activeSession?.description, reason: activeSession?.reason, citizenName: activeSession?.citizenName, fullName: activeSession?.fullName, phone: activeSession?.phone, cccd: activeSession?.cccd, cccdDate: activeSession?.cccdDate, cccdPlace: activeSession?.cccdPlace, address: activeSession?.address, leaderName: activeSession?.leaderName, leaderPosition: activeSession?.leaderPosition, status: activeSession?.status, createdAtTicket: activeSession?.createdAt, office: activeSession?.office, kioskId: 'KIOSK-TIEP-DAN-01', overall, criteria: criterionRatings, reasons: selectedReasons, comment: comment.trim(), createdAt: new Date().toISOString() };
    try {
