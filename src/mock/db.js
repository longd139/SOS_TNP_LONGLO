...
// ============================================================
// 2. NEIGHBORHOODS (45 Khu phố cho 1 màn hình điều hành)
// ============================================================
export const neighborhoods = Array.from({ length: 45 }, (_, i) => {
  const num = i + 1;
  const code = `KP${String(num).padStart(2, '0')}`;
  const id = `KP-${String(num).padStart(2, '0')}`;
  return {
    id,
    code,
    name: `Khu phố ${num}`,
    centerLatitude: 10.84 + (i % 8) * 0.003,
    centerLongitude: 106.78 + Math.floor(i / 8) * 0.003,
    status: "ACTIVE"
  };
});
